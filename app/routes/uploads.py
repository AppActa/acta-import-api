...
@router.post(
        ...
        responses = {
            200: {'description': 'Arquivo salvo com sucesso'},
            400: {'description': 'Arquivo iválido ou vazio'},
...
async def post_csv(
    ...
    arquivo: UploadFile = File(..., description='Arquivo .CSV')
):
    """
    ...
    planilha = Planilha(
        ...
        id_ciclo = id_ciclo,
        tipo_arquivo = TipoArquivo.CSV
    )
    
    return await upload_csv(arquivo, planilha)
...
async def post_xlsx(
    id_empresa: int = Form(..., description='ID da empresa no sistema ACTA', examples=[1]),
    id_ciclo: int = Form(..., description='ID do ciclo de PDCA destino no sistema ACTA', examples=[6]),
    arquivo: UploadFile = File(..., description='Arquivo .XLSX')
):
    """
    ...
    planilha = Planilha(
        id_empresa = id_empresa,
        id_ciclo = id_ciclo,
        tipo_arquivo = TipoArquivo.XLSX
    )
    
    return await upload_xlsx(arquivo, planilha)