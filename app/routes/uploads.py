...
async def post_pdf(
    ...
    """
    Importa um arquivo .pdf vinculado a um ciclo PDCA.
    
    - Valida a extensão do arquivo
    - Captura dados de texto das páginas do arquivo
    - Persiste dados no MongoDB associados a empresa e ao ciclo
    """
    ...
    documento = Documento(
        id_empresa = id_empresa,
        id_ciclo = id_ciclo,
        tipo_documento = TipoDocumento.PDF,
        contexto = contexto,
        nome_documento = arquivo.filename
    )
    
    return await upload_pdf(arquivo, documento)

@router.post(
        '/pptx/',
        summary='Upload de PPTX',
        response_description='Confirmação de persistência',
        responses = {
            200: {'description': 'Documento salvo com sucesso'},
            400: {'description': 'Documento iválido ou vazio'},
...
async def post_pptx(
    ...
    """
    Importa um arquivo .pptx vinculado a um ciclo PDCA.
    
    - Valida a extensão do arquivo
    - Captura dados de texto dos slides do arquivo
    - Persiste dados no MongoDB associados a empresa e ao ciclo
    """
    ...
    documento = Documento(
        id_empresa = id_empresa,
        id_ciclo = id_ciclo,
        tipo_documento = TipoDocumento.PPTX,
        contexto = contexto,
        nome_documento = arquivo.filename
    )
    
    return await upload_pptx(arquivo, documento)